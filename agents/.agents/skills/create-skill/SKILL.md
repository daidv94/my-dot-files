...
```yaml
---
name: skill-name      # <name> — lowercase, hyphens, max 64 chars
description: >-         # max 1024 chars, non-empty — see below
  ...
---
```

**Name** —  The name must be meaningful, lowercase, hyphenated, and under 64 characters.
Avoid generic names like `helper`, `utils`, or `workflow`. Use a single term per concept (e.g., `create-issue` not `create-ticket`).
